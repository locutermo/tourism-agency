...
export const FlipCard = ({name,description, img, flag,size }) => {
    ...
    return <div className={`group [perspective:1000px] h-52 md:h-72 lg:h-96  cursor-pointer`}>
        <div className="relative h-full w-full rounded-xl shadow-xl transition-all duration-700 [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)]">
            <div className="absolute inset-0">
                <Image src={size ? img[size] : (img.sm || img.md)} alt={`Foto de ${name}`} fill sizes='100%' className='rounded-xl object-cover shadow-xl shadow-black/10 ' />
            </div>
            <div className="absolute inset-0 h-full w-full rounded-xl px-12 text-center bg-black/40 text-slate-200 [transform:rotateY(180deg)] [backface-visibility:hidden]">
                <div className="flex min-h-full flex-col items-center justify-center">
                    ...
                    <p className="text-xs my-1 md:my-3 md:text-base overflow-x-auto text-ellipsis h-[90px] md:h-[140px]">{description}</p>
